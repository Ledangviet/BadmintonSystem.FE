...
export class AdminMainService {
  ...
  updateYardType(id: string, name: string) {
    let url = `yard-types/{yardTypeId}?id=${id}`;
    return this.apiClient.put<BaseResponseModel>(url, { id: id, name: name })
  }

  addYardType(name: any){
    return this.apiClient.post<BaseResponseModel>('yard-types',name);
  }

  removeYardType(ids: string[]){
    return this.apiClient.delete<BaseResponseModel>("yard-types", ids);
  }
